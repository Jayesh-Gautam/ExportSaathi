"""
Unit tests for Finance API Router

Tests the finance router endpoints in isolation without importing main.py
...
# Import only the finance router
...
@pytest.fixture
def mock_finance_analysis(mock_working_capital, mock_rodtep_benefit):
    ...
    return FinanceAnalysis(
        ...
        pre_shipment_credit=PreShipmentCredit(
            ...
            interest_rate=8.5,
            tenure_days=90,
            requirements=["Valid export order", "Company registration"]
        ),
        rodtep_benefit=mock_rodtep_benefit,
        gst_refund=GSTRefund(
            estimated_amount=18000.0,
            timeline_days=45,
            requirements=["GST LUT filed", "Shipping bill filed"]
        ),
        ...
        currency_hedging=CurrencyHedging(
            recommended=True,
            strategies=["Forward contract for 50-70% of order value"],
        ...
        financing_options=[
            FinancingOption(
                ...
                interest_rate=8.5,
                tenure="90 days",
                eligibility="Valid export order"
            )
        ]
    )
...
def test_calculate_rodtep_empty_hs_code(client):
    ...
    with patch('routers.finance.get_db') as mock_db:
        ...
        # Pydantic validation returns 422 for empty strings after stripping
        assert response.status_code == 422
